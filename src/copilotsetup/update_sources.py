...
def _get_upstream_branch(path: Path) -> str | None:
    ...
    result = _run_git(["rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{u}"], path)
...
def check_source(source: ConfigSource) -> SourceUpdateInfo:
    ...
    if not _is_git_checkout(source.path):
        info.status = STATUS_NOT_GIT
        info.detail = "not a git checkout"
        return info

    fetch = _run_git(["fetch", "--quiet"], source.path, timeout=60.0)
...
def apply_source(info: SourceUpdateInfo) -> tuple[bool, str]:
    ...
    result = _run_git(["pull", "--ff-only"], info.path, timeout=120.0)
    if result.returncode == 0:
